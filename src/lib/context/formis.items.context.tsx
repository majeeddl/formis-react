...
const FormisItemsContext = createContext<any[]>([]);
const FormisItemsDispatchContext = createContext<Dispatch<TFormisItemsAction>>({} as any);
...
export const FormisItemsContextProvider = ({ children, useFormis }: any) => {
  const [items, dispatch] = useReducer(formisItemsReducer, useFormis?.items);

  useFormis.getItems = () => items;

  return (
    <FormisItemsContext.Provider value={items}>
      <FormisItemsDispatchContext.Provider value={dispatch}>{children}</FormisItemsDispatchContext.Provider>
    </FormisItemsContext.Provider>
  );
};

export const useFormisItems = () => {
  const items = useContext(FormisItemsContext);
  return items;
};

export const useFormisItemsDispatch = () => {
  const dispatch = useContext(FormisItemsDispatchContext);
  return dispatch;
};
